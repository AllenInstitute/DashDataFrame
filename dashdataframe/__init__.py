...
def highlight_points(dff, selectedData, ids):
    ...
    dff["outline_width"] = 0.5
    if selectedData:
        selected_mesh_ids = np.array(
    ...
    if ids:
        ...
        dff.loc[
            dff.index.isin(list_ids) & dff.index.isin(selected_mesh_ids),
            "outline_color",
        ] = "orchid"
...
def configure_app(
    ...
    @app.callback(
        dash.dependencies.Output("link", "href"),
        [
            dash.dependencies.Input("indicator-graphic", "selectedData"),
            dash.dependencies.Input("id_list", "value"),
        ],
    ...
    if add_umap:
        ...
        @app.callback(
            dash.dependencies.Output("umap_metrics", "values"),
            [
        ...
        @app.callback(
            dash.dependencies.Output("umap_link", "href"),
            [
    ...
    if add_clustering == True:
        ...
        @app.callback(
            dash.dependencies.Output("dendro-graphic", "figure"),
            [
        ...
        def update_dendro_graph(num_clusters, selectedData, selected_metrics, norm):
            ...
            dendro = ff.create_dendrogram(
                data,
                linkagefun=lambda x: shc.linkage(data, "ward", metric="euclidean"),
                color_threshold=color_thresh,
            )
            dendro["layout"].update(
                {"height": 600, "xaxis": {"automargin": True, "showticklabels": False}}
            )
            return dendro
        ...
        def update_cluster_graph(
            ...
            if selectedData:
                outline_color, outline_width = highlight_points(
                    app._df, selectedData, id_type
                )
            ...
            if cluster_clicks is not None:
                if cluster_clicks != app._prev_cluster_clicks:
                    if "Agglomerative" in method:
                        ...
                        if "Ward" in method:
                            linkage = "ward"
                        clusters = calculate_new_aggclustering(
                            app._df,
                            int(num_clusters),
                            selected_metrics,
                            linkage,
                    ...
                    if method == "KMeans":
                        clusters = calculate_new_kmeans(
                            app._df, int(num_clusters), selected_metrics, znorm=znorm
                        )
                    if method == "Spectral Clustering":
                        clusters = calculate_new_spectclustering(
                            app._df, int(num_clusters), selected_metrics, znorm=znorm
                        )
        ...
        @app.callback(
            dash.dependencies.Output("cluster_metrics", "values"),
            [
        ...
        @app.callback(
            dash.dependencies.Output("cluster_link", "href"),
            [
                dash.dependencies.Input("cluster-graphic", "selectedData"),
                dash.dependencies.Input("id_list", "value"),
            ],
